...
serialport::serialport(QWidget *parent) : QMainWindow(parent)
{
    ...
    serial_attr_combox[1]->setCurrentIndex(2);
    ...
    plot->setTitle(tr("温度"));
    plot->setAxisScale( QwtPlot::yLeft, -20.0, 30 );
    plot->setAxisScale( QwtPlot::xBottom, 0.0, 500, 50);
    ...
    QwtSymbol *symbol1 = new QwtSymbol( QwtSymbol::Ellipse,
        QBrush( Qt::green ), QPen( Qt::blue, 2 ), QSize( 2, 2 ) );
    curve[1]->setSymbol( symbol1 );
...
void serialport::on_refresh()
{
    //widget[1]->update();
#if 0
    static float ftmp = 21.1;
    ftmp += 0.2;
    QString str = QString("{{%f\t%f\t}}").arg(ftmp).arg(ftmp+5);
    temp.append(str);
#endif
...
void serialport::readMyCom()
{
    ...
    while (temp.contains("{{") && temp.contains("}}"))
    {
        ...
        temp1 = temp1.left(temp.indexOf("\t"));
        qDebug("%s\n", temp1.data());
        temperature1 = temp1.toFloat();

        temp2 = temp2.right(temp.indexOf("\t"));
        temp2.remove(0, 2);
        temp2.remove(temp2.length()-2, 2);
        qDebug("%s\n", temp2.data());
        temperature2 = temp2.toFloat();

        temp.clear();
        QDateTime time = QDateTime::currentDateTime();
        QString datatime = time.toString("yyyy-MM-dd HH:mm:ss");

        //temperature1 += temperature1*1.2 +1;
        //if(temperature1>20)
            //temperature1--;
        //数据保存到数据库
        ...
        curve[0]->attach( plot );
        //plot->replot();
        static QPolygonF points1;
        points1 << QPointF( i, temperature1-3 );
        curve[1]->setSamples( points1 );
        curve[1]->attach( plot );
        plot->replot();
        //更新数据数量
        i++;
...
void serialport::save_as_button_clicked()
{
    ...
    else if (file.suffix() == "pdf")
    {
        ...
        while (query->next())
        {
            painter.drawText(20,20*i,QString("时间：%1   位置点1温度：%2  位置点2温度：%3\r\n").arg(query->value(3).toString()).arg(query->value(1).toString()).arg(query->value(2).toString()));
            if ((i*20) > 1000)
            {
                i = 0;
                printer.newPage();
